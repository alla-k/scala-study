...
class BinaryTreeSuite(_system: ActorSystem) extends TestKit(_system) with FunSuiteLike with Matchers with BeforeAndAfterAll with ImplicitSender
{
  ...
  test("multiple inserts") {
    ...
    topNode ! Insert(testActor, id = 5, 3)
    topNode ! Insert(testActor, id = 6, 2)
    ...
    topNode ! Contains(testActor, id = 9, 2)
  ...
  test("instruction example2") {
    ...
    val ops = List(
      ...
      Contains(requesterRef, id=80, 1),
      Contains(requesterRef, id=70, 2),
      Insert(requesterRef, id=60, 5),
      Insert(requesterRef, id=65, 4),
      Remove(requesterRef, id=90, 2),
      Contains(requesterRef, id=121, 5),
      Remove(requesterRef, id=123, 5),
      Contains(requesterRef, id=124, 4)
    )
    ...
    val expectedReplies = List(
      ...
      OperationFinished(id=20),
      ContainsResult(id=50, false),
      OperationFinished(60),
      OperationFinished(65),
      ContainsResult(id=70, true),
      ContainsResult(id=80, false),
      OperationFinished(id=90),
      OperationFinished(id=100),
      ContainsResult(id=121, true),
      OperationFinished(id=123),
      ContainsResult(id=124, true)
    )
